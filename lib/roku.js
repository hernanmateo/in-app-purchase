    ...
    request.get(options, function (error, response, body) {
        ...
        if (res.expirationDate) {
            res.expirationDate = new Date(
                parseInt(res.expirationDate.substr(6), 10)
            ).getTime();
        }
        res.originalPurchaseDate = new Date(
            parseInt(res.originalPurchaseDate.substr(6), 10)
        ).getTime();
        res.purchaseDate = new Date(
            parseInt(res.purchaseDate.substr(6), 10)
        ).getTime();
