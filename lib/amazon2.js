...
module.exports.getPurchaseData = function (purchase, options) {
    ...
    if (
        options &&
        options.ignoreExpired &&
        purchase.cancelDate &&
        purchase.cancelDate <= now
    ) {
        return [];
    }
...
function send(path, errorMap, cb) {
    request.get(path, function (error, response, body) {
        ...
        try {
            ...
            exc.message +=
                ": " + path + " [status: " + response.statusCode + "]";
            return cb(exc);
        }
        cb(null, res);
    });
