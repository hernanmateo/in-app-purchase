...
module.exports.readConfig = function (configIn) {
    ...
    if (configIn.facebookAppSecret && configIn.facebookAppId) {
        fb.setAccessToken(
            configIn.facebookAppId + "|" + configIn.facebookAppSecret
        );
    }
...
module.exports.validatePurchase = function (
    ...
    if (oneTimeAppAccessToken) {
        verbose.log(
            "<Facebook> Using dynamic app access token:",
            oneTimeAppAccessToken
        );
    ...
    if (!urlbase64.validate(encodedSign)) {
        ...
        cb(
            new Error(
                "failed to validate purchase: signature is not a valid urlsafe-base64 form"
            ),
            {
                status: constants.VALIDATION.FAILURE,
    ...
    if (purchaseObj.status !== "completed") {
        ...
        cb(
            new Error(
                "failed to validate purchase: payments not completed: " +
                    purchaseObj.status
            ),
            {
                status: constants.VALIDATION.FAILURE,
    ...
    fb.api(
        purchaseObj.payment_id + "?",
        "get",
        requestParams,
        function (paymentRes) {
            if (!paymentRes || paymentRes.error) {
                verbose.log(
                    "<Facebook> Failed to call graph api: ",
                    paymentRes
                );
                paymentRes = paymentRes || {};
                cb(
                    new Error(
                        "failed to validate purchase: graph api call error: " +
            ...
            if (
                purchaseObj.amount !== paymentRes.actions[0].amount ||
                ...
                verbose.log(
                    "<Facebook> Receipt info does not match with info from facebook"
                );
                cb(
                    new Error(
                        "failed to validate purchase: payment information not matching"
                    ),
                    {
                        status: constants.VALIDATION.FAILURE,
